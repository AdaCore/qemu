...
#define IPIC_MASK(n) \
    ((ipic_sources[n].bit >= 0) ? 1 << (31 - ipic_sources[n].bit) : 0)
...
static uint32_t *ipic_get_register(IPICState *s,
    IPICDirection direction, target_phys_addr_t address, int size,
    int *reg_offset)
{
    uint32_t *reg;
    int offset;
...
static void ipic_compute_io(target_phys_addr_t address, int size,
...
static uint64_t ipic_read(void *state, target_phys_addr_t address,
                           unsigned size)
{
    IPICState *s = state;
...
static void ipic_write(void *state, target_phys_addr_t address, uint64_t value,
                        unsigned size)
{
    IPICState *s = state;
 ...
 * inputs are returned */
qemu_irq *ipic_init(MemoryRegion *address_space, target_phys_addr_t base_addr,
                    qemu_irq out_irqs[IPIC_OUTPUT_SIZE])
{
    ...
    memory_region_init_io(&s->mem, &ipic_ops, s, "ipic", IPIC_MEM_SIZE);

...
/*
 * FIXME: We don't use migration but this device use register_savevm(..)
 * The best way is to use qdev->vmsd to use migration.. So lets include
 * migration/register.h for the time beeing until this is fixed.
 */
...
/* See the comment above in the include section */
static SaveVMHandlers ipic_savevm = {
    .save_state = ipic_save,
    .load_state = ipic_load,
};
                    ...
                    qemu_irq out_irqs[IPIC_OUTPUT_SIZE])
{
    ...
    /* FIXME: See the comment above in the include section */
    register_savevm_live("IPIC", 0, IPIC_VERSION, &ipic_savevm, s);

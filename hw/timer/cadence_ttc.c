...
static void zynqmp_ttc_realize(DeviceState *dev, Error **errp)
{
    CadenceTTCState *s = CADENCE_TTC(dev);
    int i;
...
static void zynqmp_ttc_class_init(ObjectClass *klass, const void *data)
{
    DeviceClass *dc = DEVICE_CLASS(klass);

    dc->vmsd = &vmstate_cadence_ttc;
    dc->realize = zynqmp_ttc_realize;
}
...
static const TypeInfo zynqmp_ttc_info = {
    ...
    .instance_size  = sizeof(CadenceTTCState),
    .class_init = zynqmp_ttc_class_init,
};

                                           ...
                                           GnatBusPacket_Register *reg)
{
    ...
    qdev = qdev_new("gnatbus-device");
    pdev = (typeof(pdev))qdev;
    pdev->qbdev = qbdev;

    memcpy(&qbdev->info, reg, sizeof(*reg));
    sysbus_realize_and_unref(SYS_BUS_DEVICE(qdev), &error_fatal);
...
static void gnatbus_device_class_init(ObjectClass *klass, const void *data)
{

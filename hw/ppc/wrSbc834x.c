                         ...
                         const uint32_t       cpu_freq)
{
    ...
    /* allocate RAM */
    memory_region_init_ram(ram, NULL, "sbc834x.ram", args->ram_size,
                           &error_abort);
    ...
    if (serial_hd(0)) {
        serial_mm_init(ccsr_space, SERIAL0_OFFSET, 0, devirqs[9], 9600,
                       serial_hd(0), DEVICE_BIG_ENDIAN);
    }

    if (serial_hd(1)) {
        serial_mm_init(ccsr_space, SERIAL1_OFFSET, 0, devirqs[10], 9600,
    ...
    kernel_size = load_elf(args->kernel_filename, NULL, NULL, NULL,
                           &kernel_pentry, &kernel_lowaddr, NULL, NULL, 1,
                           PPC_ELF_MACHINE, 0, 0);
    if (kernel_size < 0) {
...
static void sbc834x_generic_machine_init(MachineClass *mc)
{
    mc->desc = "WindRiver SBC834x board";
    mc->max_cpus = MAX_CPUS;
    mc->init = sbc834x_init;
    mc->default_cpu_type = POWERPC_CPU_TYPE_NAME("mpc8349");
}

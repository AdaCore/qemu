...
static void main_cpu_reset(void *opaque)
{
    ...
    tlb1->mas2    = 0xfffff000 & qemu_target_page_mask();
    tlb1->mas7_3  = 0xfffff000 & qemu_target_page_mask();
...
static void fsl_e500_init(fsl_e500_config *config, MachineState *machine)
{
    ...
    memory_region_init_ram(ram, NULL, "3041.ram", machine->ram_size,
                           &error_abort);
    ...
    /* MPIC */
    mpicdev = init_mpic(machine, ccsr_space, irqs);
    ...
    /* Serial */
    if (serial_hd(0)) {
        serial_mm_init(ccsr_space, P3041DS_SERIAL0_REGS_BASE, 0,
                       qdev_get_gpio_in(mpicdev, config->serial_irq),
                       115200, serial_hd(0), DEVICE_BIG_ENDIAN);
    }

    if (serial_hd(1)) {
        serial_mm_init(ccsr_space, P3041DS_SERIAL1_REGS_BASE, 0,
                       qdev_get_gpio_in(mpicdev, config->serial_irq),
    ...
    if (kernel_filename) {
        ...
        kernel_size = load_elf(kernel_filename, NULL, NULL, NULL, &elf_entry,
                               &elf_lowaddr, NULL, NULL, ELFDATA2MSB,
                               PPC_ELF_MACHINE, 0, 0);

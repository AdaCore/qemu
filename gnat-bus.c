...
static GnatBusPacket *gnatbus_receive_packet_sync(GnatBus_Device *qbdev)
{
...
#endif
    {
        TCPCharDriver *s_socket = qbdev->chr.chr->opaque;
        qio_channel_set_blocking(s_socket->ioc, true, NULL);
        trace_gnatbus_set_blocking(true);
    }
 ...
 fail:
    ...
    {
        TCPCharDriver *s_socket = qbdev->chr.chr->opaque;
        qio_channel_set_blocking(s_socket->ioc, false, NULL);
        trace_gnatbus_set_blocking(false);
    }
    return packet;
}

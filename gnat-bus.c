...
static GnatBusPacket *gnatbus_receive_packet_sync(GnatBus_Device *qbdev)
{
   ...
   /* Read first part of the packet to get its complete size */
    ...
    packet = qemu_realloc(packet, packet->size);
                                        ...
                                        int             size)
{
    ...
    qbdev->curr_packet = qemu_realloc(qbdev->curr_packet, packet_size);
...
static int gnatbus_init(const char *optarg)
{
    ...
    /* Connect chardev */
    chr = qemu_chr_open("GnatBus_Chr", buf, NULL);
...
void gnatbus_save_optargs(const char *optarg)
{
    ...
    if (g_qbmaster == NULL) {
        g_qbmaster = qemu_mallocz(sizeof *g_qbmaster);
    }

    g_qbmaster = qemu_malloc(sizeof(*g_qbmaster));
    g_qbmaster->optarg = qemu_strdup(optarg);
